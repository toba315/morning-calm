...
## 2026-06-27 管理画面の反映条件と表示改善

管理画面で「取得完了」と出ても中身が変わっていないケースを区別できるようにし、反映件数の制約も緩めた。

### 変更内容

- `POST /api/fetch` の前後を比較し、候補ニュースが同じなら「新しいニュースはありませんでした」と表示するようにした。
- `POST /api/fetch` の応答に `changed` と `message` を追加した。
- `POST /api/publish` は 1 件から 5 件まで反映できるようにした。
- 反映件数はサーバー応答の文言にそのまま出すようにした。
- 管理画面の空き枠には、落ち着いた短い格言を表示するようにした。
- フロント側は 5 件未満でも、そのまま公開済み件数として表示するようにした。

### 確認

- `POST /api/fetch` で、同じ内容なら「新しいニュースはありませんでした」と返ることを確認した。
- `POST /api/publish` で 3 件の反映が通ることを確認した。
- `node --check admin.js`: OK
- `node --check app.js`: OK
- `node --check scripts/admin-server.mjs`: OK
